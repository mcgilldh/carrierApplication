...
if (!empty($vars['carr_id'])) {
    ...
    $ids = array();
    $getCarrIdQuery = "SELECT textile_carr_id FROM textile_carr WHERE vt_tracking='".$vars['carr_id']."'";
    $carrIdResponse = mysqli_query($mysqli, $getCarrIdQuery);
    $carrId = mysqli_fetch_assoc($carrIdResponse)['textile_carr_id'];
    
    $getIdsQueryString = "SELECT textile_inst_id, textile_id, vt_tracking FROM textile_instance WHERE textile_instance.textile_carr_id='".$carrId."'";
    ...
    for ($i=0; $i<count($ids); $i++) {
        ...
        $getImgUrlString = "SELECT img_path FROM img_detail INNER JOIN img_hdr ON img_detail.img_hdr_id = img_hdr.img_hdr_id WHERE img_hdr.textile_inst_id='".$textile_inst_id."' AND img_detail.img_type_cd='FL'";
        ...
        $getOwnerString = "SELECT own_id FROM textile_instance WHERE textile_inst_id='".$textile_inst_id."'";
        $queryOwner = mysqli_query($mysqli, $getOwnerString);
        $curOwner = mysqli_fetch_assoc($queryOwner)['own_id'];
        
        //Get the description
        $getDescrString = "SELECT textile_descr FROM textile WHERE textile.textile_id=".$ids[$i][1];
        ...
        $getProvString = "SELECT own_id, own_type_cd, own_st_dt, own_end_dt, own_term_cd, provenance_id FROM provenance WHERE provenance.textile_inst_id=".$ids[$i][0];
        ...
        $getTagString = "SELECT textile_meta_tag_id, textile_meta_tag FROM textile_meta_tag WHERE textile_meta_tag.textile_id=".$ids[$i][1];
...
//This is the case where we are trying to get the lists of types, owners and terms available
    ...
    $getAttributeString = "SELECT cd_attr_nm, cd_id, cd_valu FROM codec";
