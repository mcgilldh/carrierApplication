...
function generateTags(tagData, inst_id) {
    ...
    txt += "<div class='infoPointTagList' style='height: 250px'>"; 
...
function populateInfo(data) {   
	...
	//New entry
	txt += "<div class='info' id='info"+inst_id+"'>";
	...
	txt += "<img id='img"+inst_id+"' src='"+ data[i].imgUrl +"'>";
	txt += "</div>";

	//Main text of the textile description
	txt += "<h1>Textile Description</h1> ";
	txt += "<textarea id = 'description" + inst_id + "' cols='70' rows='10'>" + data[i].description + "</textarea>";

	txt += "<div class='tagMain'>";
	txt += generateTags(data[i].tags, inst_id);

	txt += "<a href='#'><div class='submitChanges' onclick='submitChanges(" + inst_id + ");return false;'>Submit changes</div></a>";	txt += "</div><br><br>";
	...
	$(function () { $( "#img"+ data[i].textile_inst_id).elevateZoom(); });
...
/*Submits the modifications that have been made to the requested carrier
  Arguments: id - the textile instance ID
...
function gotoCarrier(carrierID) {
    window.location.href = "carrier.php?carrID=" + carrierID;
}
